...
| `sda-app-control` | Binary authorization / application control — Ed25519-signed policy verification (`policy.rs`), Monitor mode (Phase-4 default; logs allow / deny), Enforce mode with single-step `DualControlRollback` (`enforce.rs`), bus events on policy apply / decision (Phase 4; default off, Phase-4 PAL stubs return `NotSupported`; macOS Santa stub) |
