...
| Push to `main` | Above + `make test-integration` + `make test-e2e-all` | Medium |
| Manual dispatch | Above + `make benchmark-ci` (optional) | Slow |

Docs-only changes (`docs/**`, `*.md`, `LICENSE`) skip CI entirely. The full
suite is also runnable on demand via the `Run workflow` button on the CI
workflow page (toggle `run_full_suite` and/or `run_benchmark`).
