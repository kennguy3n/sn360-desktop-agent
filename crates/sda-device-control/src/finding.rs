...
pub struct Finding {
    ...
    /// determined by `kind` (see SCHEMAS.md § 5.3).
...
/// Forensic re-walk reference (SCHEMAS.md § 5.1).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
...
/// Hard cap on `Finding.plain_english` (SCHEMAS.md § 2.4).
pub const FINDING_PLAIN_ENGLISH_MAX: usize = 512;

/// Hard cap on `Finding.evidence` serialised size (SCHEMAS.md § 2.4).
...
impl Finding {
    /// Validate the structural invariants from SCHEMAS.md § 5.2.
    ///
