...
/// Production hooks backed by a real Ed25519 key rotation set
/// delivered from the TRDS bundle. Each key is stored as a
/// `(key_id, VerifyingKey)` pair; the router looks up the job's
/// `key_id` and verifies the Ed25519 signature over the RFC 8785
/// canonical pre-image of the job (with `signature` set to empty).
pub struct ProductionHooks {
    keys: std::collections::HashMap<String, ed25519_dalek::VerifyingKey>,
    /// Actions the agent's tier permits. `None` = permit all.
    permitted_actions: Option<std::collections::HashSet<ActionKind>>,
...
impl ProductionHooks {
    ...
    /// Register a local ephemeral key ID generated by the auto-
    /// remediation supervisor.
    pub fn register_local_ephemeral_key(&mut self, key_id: String) {
        self.local_ephemeral_key_ids.insert(key_id);
    }

    /// Add or rotate a signing key at runtime (e.g. on TRDS bundle
    /// refresh).
    ...
    /// verification: RFC 8785 canonical JSON of the job with the
    /// `signature` and `additional_signatures` fields blanked.
    fn signing_pre_image(job: &SignedActionJob) -> Result<Vec<u8>, JobRefused> {
        let mut value = serde_json::to_value(job).map_err(|_| JobRefused::BadSignature)?;
        ...
        canonicalize(&value).map_err(|_| JobRefused::BadSignature)
    }
}

impl JobValidationHooks for ProductionHooks {
    fn verify_signature(&self, job: &SignedActionJob) -> Result<(), JobRefused> {
        let key = self.keys.get(&job.key_id).ok_or(JobRefused::UnknownKeyId)?;

        let pre_image = Self::signing_pre_image(job)?;
    ...
    fn verify_additional_signature(
        ...
        let key = self
            .keys
            .get(&additional.key_id)
            .ok_or(JobRefused::UnknownKeyId)?;

        let pre_image = Self::signing_pre_image(job)?;

        let sig_bytes: [u8; 64] = additional
            .signature
            .as_slice()
            .try_into()
            .map_err(|_| JobRefused::BadSignature)?;
    ...
    fn is_local_ephemeral_key(&self, key_id: &str) -> bool {
        self.local_ephemeral_key_ids.contains(key_id)
    }
}
