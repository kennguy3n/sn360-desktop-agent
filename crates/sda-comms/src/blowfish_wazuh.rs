//! Wazuh-compatible Blowfish-CBC implementation.
//!
//! The standard `blowfish` Rust crate limits keys to 56 bytes, but Wazuh's
//! OpenSSL-based implementation uses 64-byte keys (two concatenated MD5 hex
//! digests). This module implements Blowfish with support for keys up to
//! 72 bytes, matching OpenSSL's `BF_set_key` behavior.
...
impl Blowfish {
    /// Create a new Blowfish cipher with the given key (4–72 bytes).
    pub fn new(key: &[u8]) -> Self {
        assert!(
            (4..=72).contains(&key.len()),
            "Blowfish key must be 4–72 bytes, got {}",
            key.len()
        );
        ...
        // XOR key bytes cyclically into the P-array.
        let mut j = 0usize;
        for i in 0..18 {
            let mut data = 0u32;
            for _ in 0..4 {
        ...
        // Expand key: encrypt successive all-zero blocks and replace P then S.
        let mut l = 0u32;
        let mut r = 0u32;
    ...
    /// Encrypt a single 8-byte block in place (as two u32 halves).
    fn encrypt_pair(&self, l: &mut u32, r: &mut u32) {
        let mut xl = *l;
        let mut xr = *r;
        for i in 0..16 {
            xl ^= self.p[i];
    ...
    /// Decrypt a single 8-byte block in place (as two u32 halves).
    fn decrypt_pair(&self, l: &mut u32, r: &mut u32) {
        let mut xl = *l;
        let mut xr = *r;
        for i in (2..18).rev() {
            xl ^= self.p[i];
...
pub fn bf_cbc_decrypt(bf: &Blowfish, ciphertext: &[u8]) -> Vec<u8> {
    ...
    for i in (0..buf.len()).step_by(8) {
        ...
        let block: &mut [u8; 8] = (&mut buf[i..i + 8]).try_into().unwrap();
...
// ---------------------------------------------------------------------------
// Standard Blowfish initial P-array and S-box values (derived from pi).
// These are identical across all conforming implementations.
// ---------------------------------------------------------------------------

#[rustfmt::skip]
const INITIAL_P: [u32; 18] = [
...
mod tests {
    ...
    fn test_known_vector() {
        // Standard Blowfish test vector: key = all zeros (8 bytes),
        // plaintext = all zeros → ciphertext = 0x4EF997456198DD78
    ...
    #[test]
    fn test_64_byte_key() {
        // Verify that a 64-byte key is accepted and produces consistent results.
        let key = b"0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
        let bf = Blowfish::new(key);
        let mut block = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08];
        let original = block;
        bf.encrypt_block(&mut block);
    ...
    fn test_cbc_roundtrip() {
        let key = b"test-key-for-blowfish-cbc-mode!!";
        let bf = Blowfish::new(key);
        let plaintext = b"Hello, Wazuh server! This is a test message.";
        let ciphertext = bf_cbc_encrypt(&bf, plaintext);
        let decrypted = bf_cbc_decrypt(&bf, &ciphertext);
        // Decrypted may have trailing zero-padding; compare only the original length.
        assert_eq!(&decrypted[..plaintext.len()], &plaintext[..]);
    }

    #[test]
    fn test_cbc_empty() {
        let bf = Blowfish::new(b"keykey");
        let ct = bf_cbc_encrypt(&bf, b"");
        assert!(ct.is_empty());
        let pt = bf_cbc_decrypt(&bf, &ct);
        assert!(pt.is_empty());
    }

    #[test]
    fn test_cbc_block_aligned() {
        let bf = Blowfish::new(b"eightkey");
        let plaintext = b"exactly!"; // 8 bytes, block-aligned
        let ct = bf_cbc_encrypt(&bf, plaintext);
        assert_eq!(ct.len(), 8);
        let pt = bf_cbc_decrypt(&bf, &ct);
        assert_eq!(&pt[..8], &plaintext[..]);
    }
}
