...
dropping the `legacy-siem` Cargo feature. The SN360 native
protocol will become the default in a future release; the legacy
adapter remains available for fleets that still depend on it.
...
`NeverOverwrite="yes"` so operator edits survive upgrades.
...
place. The updater re-reads the installed version after each
successful install, so a single manifest will not trigger a
re-download loop.
...
- **Metrics:** benchmark thresholds and CI results are tracked in
  [`benchmarks.md`](./benchmarks.md). The regression gate
  (`make benchmark-ci`) fails if idle RSS > 15 MB, idle CPU
  > 0.1 %, binary > 7 MB, or FIM burst peak > 3 %.
...
- TLS 1.3 + certificate pinning are available via the SN360
  ...
  `server.enhanced.serialization: msgpack`,
  `server.protocol: http2`). This is opt-in today and will
